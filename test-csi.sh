...
for (( i=0; i<${#USERNAMES[*]}; i++ )); do
    ...
    http POST https://localhost:3000/api/v1/user/create \
...
for (( i=0; i<${#USERNAMES[*]}; i++ )); do
    ...
    http POST https://localhost:3000/api/v1/user/login username=${USERNAMES[$i]} password=${PASSWORDS[$i]} --verify=no -b
    #echo "Logging in with email. ${EMAILS[$i]}|${PASSWORDS[$i]}"
    #http POST https://localhost:3000/api/v1/user/login email=${EMAILS[$i]} password=${PASSWORDS[$i]} --verify=no -b
done

# Login and store tokens
ADMIN_TOKEN=$(http POST https://localhost:3000/api/v1/user/login username=b0nes password=12345678 --verify=no -b)
[ "$ADMIN_TOKEN" ] ||  echo "Token test failed."
USER_TOKEN=$(http POST https://localhost:3000/api/v1/user/login username=user password=18273645 --verify=no -b)
...
http DELETE https://localhost:3000/api/v1/user/epicfail Authorization:${USER_TOKEN} --verify=no -b
echo "Deleting user epicfail with admin credentials."
http DELETE https://localhost:3000/api/v1/user/epicfail Authorization:${ADMIN_TOKEN} --verify=no -b

# Update a user
echo "Updating user \"user\", logged in as \"user\"."
http PUT https://localhost:3000/api/v1/user/user Authorization:${USER_TOKEN} firstName=Tommieboy lastName=Hagendasz --verify=no -b
echo "Updating user \"user2\", logged in as \"user\"."
http PUT https://localhost:3000/api/v1/user/user2 Authorization:${USER_TOKEN} firstName=bla --verify=no -b
echo "Updating user \"user\", logged in as \"b0nes\" (admin)."
http PUT https://localhost:3000/api/v1/user/user2 Authorization:${ADMIN_TOKEN} email=user2@supermail.nl password=Welkom2018 --verify=no -b
echo "Updating user \"user\", logged in as \"b0nes\" (admin) with bogus data."
http PUT https://localhost:3000/api/v1/user/user2 Authorization:${ADMIN_TOKEN} bla=user2@supermail.nl wtfEvenIsThis=Welkom2018 --verify=no -b


# Get a user
echo "Getting user \"user2\", logged in as \"user\" (admin)"
http GET https://localhost:3000/api/v1/user/user2 Authorization:${USER_TOKEN} --verify=no -b
echo "Getting user \"user2\", logged in as \"b0nes\" (admin)"
http GET https://localhost:3000/api/v1/user/user2 Authorization:${ADMIN_TOKEN} --verify=no -b
echo "Getting user \"user2\" with field firstname, logged in as \"user\" (admin)"
http GET https://localhost:3000/api/v1/user/user2/firstName Authorization:${USER_TOKEN} --verify=no -b
echo "Getting user \"user2\" with field firstname, logged in as \"b0nes\" (admin)"
http GET https://localhost:3000/api/v1/user/user2/firstName Authorization:${ADMIN_TOKEN} --verify=no -b
